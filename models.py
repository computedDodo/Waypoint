...
class Task(db.Model):
    ...
    # ---------------------------------------------------------
    #
    # If prerequisite_task_id is NULL:
    #     The task can be claimed normally.
    #
    # If prerequisite_task_id points to another task:
    #     The prerequisite task must be approved before this
    #     task can be claimed.
    #
    # Dependencies are intentionally campaign-scoped.
    #
    prerequisite_task_id = db.Column(
        db.Integer,
        db.ForeignKey('tasks.id'),
        nullable=True
    )

    prerequisite_task = db.relationship(
        'Task',
        remote_side=[id],
